...
namespace LSOne.DataLayer.SqlDDDataProviders
{
    public class DesignData : SqlServerDataProvider, IDesignData
    {
        ...
        public void Save(IConnectionManager entry, JscTableDesign tableDesign)
        {
            ...
            {
                var design = GetTableDesign(entry, tableDesign.DatabaseDesign, tableDesign.TableName);
                if (design == null)
                {
                    isNew = true;
        ...
        public void Save(IConnectionManager entry, JscFieldDesign fieldDesign)
        {
            ...
            {
                var design = GetFieldDesign(entry, fieldDesign.TableDesign, fieldDesign.FieldName);
        ...
        public void Save(IConnectionManager entry, JscLinkedTable linkedTable)
        {
            ...
            {
                var table = GetLinkedTable(entry, linkedTable.FromTable, linkedTable.ToTable);
        ...
        public IEnumerable<JscTableMap> GetDatabaseMaps(IConnectionManager entry, RecordIdentifier tableDesignFilterFrom = null, RecordIdentifier tableDesignFilterTo = null)
        {
            ValidateSecurity(entry, SchedulerPermissions.DatabaseMapView);
        ...
        public JscFieldDesign GetFieldDesign(IConnectionManager entry, RecordIdentifier tableDesignId, string fieldName)
        {
            ValidateSecurity(entry,
                             new[] { SchedulerPermissions.DatabaseDesignView, SchedulerPermissions.JobSubjobView, SchedulerPermissions.DatabaseMapView });
            ...
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = FieldDesignGetString + @"
                                WHERE 
                                    TableDesign = @TableDesign AND FieldName = @FieldName";

                MakeParam(cmd, "TableDesign", (Guid)tableDesignId);
                MakeParam(cmd, "FieldName", fieldName);
        ...
        public IEnumerable<JscTableDesign> GetTableDesigns(IConnectionManager entry, RecordIdentifier databaseDesignId, bool onlyWhereLinkedTablesExists)
        {
            ValidateSecurity(entry,
                             new[]
                                 {
            ...
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                if (!onlyWhereLinkedTablesExists)
                {
        ...
        private JscLinkedTable GetLinkedTable(IConnectionManager entry, RecordIdentifier fromtableId, RecordIdentifier toTableId)
        {
            ValidateSecurity(entry, new[] { SchedulerPermissions.DatabaseDesignView, SchedulerPermissions.DatabaseMapView });
            ...
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                cmd.CommandText = LinkedTableGetString + @"
                                ...
                                WHERE 
                                    lt.FromTable =  @FromTableId 
                                    AND    
                                    lt.ToTable =  @ToTableId";


                MakeParam(cmd, "FromTableId", (Guid)fromtableId);
                MakeParam(cmd, "ToTableId", (Guid)toTableId);
        ...
        public IEnumerable<JscLocation> GetLocationsUsingDatabase(IConnectionManager entry,RecordIdentifier databaseDesignId)
        {
            ValidateSecurity(entry,
        ...
        public JscTableDesign GetTableDesign(IConnectionManager entry, RecordIdentifier databaseDesignId, string tableName)
        {
            ValidateSecurity(entry,
                             new[]
                                 {
            ...
            {
                cmd.CommandText = TableDesignGetString + @"
                                    WHERE DatabaseDesign = @DatabaseDesignID AND TableName =  @TableName ";


                MakeParam(cmd, "DatabaseDesignID", (Guid)databaseDesignId);
        ...
        public void DeleteFieldDesign(IConnectionManager entry, RecordIdentifier fieldDesignID)
        {
            DeleteRecord(entry, "JscFieldDesigns", "Id", fieldDesignID, SchedulerPermissions.DatabaseDesignEdit);
        }
    }
}
