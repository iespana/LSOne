...
namespace LSOne.DataLayer.TransactionObjects.Receipts
{
    public class ReprintInfo : DataEntity, IReprintInfo
    {
        ...
        {
            ReprintInfo item = new ReprintInfo();
            Populate(item);
            return item;
        }
        ...
        protected void Populate(ReprintInfo reprint)
        {
            base.Populate(reprint);
            reprint.LineID = LineID;
            reprint.ReprintDate = ReprintDate;
            reprint.ReprintType = ReprintType;
            reprint.Staff = Staff;
            reprint.Store = Store;
            reprint.Terminal = Terminal;
        }
            ...
            try
            {
               ...
               */
                XElement xReprintInfo = new XElement("ReprintInfo",
                    ...
                    new XElement("ReprintType", Conversion.ToXmlString((int)ReprintType))
                );
                            ...
                            try
                            {
                                switch (elem.Name.ToString())
                                {
                                    ...
                                    case "Terminal":
                                        ...
                                        break;
                                    case "ReprintType":
                                        ReprintType = (ReprintTypeEnum) Conversion.XmlStringToInt(elem.Value);
                                        break;
                                }
                            }
                            catch (Exception ex)
