...
{
    public class DiscountOfferDataOLD : SqlServerDataProviderBase
    {
        ...
        private static string BasePeriodicDiscountSql
        {
            ...
            {               
                return @"select g.OFFERID as OFFERID,
                         ...
                         left join CUSTOMER c on c.ACCOUNTNUM = g.ACCOUNTRELATION and c.DATAAREAID = g.DATAAREAID
...
pl.DATAAREAID = g.DATAAREAID 
        ...
        {
            DeleteRecord(entry, "POSPERIODICDISCOUNT", "OFFERID", offerID, BusinessObjects.Permission.ManageDiscounts);
            DeleteRecord(entry, "POSPERIODICDISCOUNTLINE", "OFFERID", offerID, BusinessObjects.Permission.ManageDiscounts);

            if (type == DiscountOffer.PeriodicDiscountOfferTypeEnum.MultiBuy)
            {
                DeleteRecord(entry, "POSMULTIBUYDISCOUNTLINE", "OFFERID", offerID, BusinessObjects.Permission.ManageDiscounts);
            }
            else if (type == DiscountOffer.PeriodicDiscountOfferTypeEnum.MixMatch)
            {
                DeleteRecord(entry, "POSMMLINEGROUPS", "OFFERID", offerID, BusinessObjects.Permission.ManageDiscounts);
            }
            else if (type == DiscountOffer.PeriodicDiscountOfferTypeEnum.Promotion)
            {
                DeleteRecord(entry, "RBODISCOUNTOFFERLINE", "OFFERID", offerID, BusinessObjects.Permission.ManageDiscounts);
            }
        }
