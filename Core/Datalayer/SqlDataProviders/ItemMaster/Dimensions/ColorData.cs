...
namespace LSOne.DataLayer.SqlDataProviders.ItemMaster.Dimensions
{
    ...
    /// </summary>
    public class ColorData : SqlServerDataProviderBase, IColorData
    {
        /// <summary>
        ...
        private static void PopulateColor(IDataReader dr, ItemWithDescription color)
        {
        ...
        public virtual ItemWithDescription GetColor(IConnectionManager entry, RecordIdentifier colorID)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                var items = Execute<ItemWithDescription>(entry, cmd, CommandType.Text, PopulateColor);
                return (items.Count > 0) ? items[0] : null;
            }
        }

        private static void PopulateColorGroupLine(IDataReader dr, StyleGroupLineItem item)
        {
        ...
        public virtual List<StyleGroupLineItem> GetGroupLines(IConnectionManager entry, RecordIdentifier groupID, string sort)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                return Execute<StyleGroupLineItem>(entry, cmd, CommandType.Text, PopulateColorGroupLine);
            }
        }
        ...
        public virtual StyleGroupLineItem GetGroupLine(IConnectionManager entry, RecordIdentifier groupID, RecordIdentifier colorID)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                //ValidateSecurity(entry, BusinessObjects.Permission.CustomerView);
                ...
                var items = Execute<StyleGroupLineItem>(entry, cmd, CommandType.Text, PopulateColorGroupLine);
                return (items.Count > 0) ? items[0] : null;
            }
        }
        ...
        {
            DeleteRecord(entry, "RBOCOLORS", "COLOR", colorID, BusinessObjects.Permission.ColorSizeStyleEdit);
        }
        ...
        {
            DeleteRecord(entry, "RBOCOLORGROUPTABLE", "COLORGROUP", colorGroupID, BusinessObjects.Permission.ColorSizeStyleEdit);
            DeleteRecord(entry, "RBOCOLORGROUPTRANS", "COLORGROUP", colorGroupID, BusinessObjects.Permission.ColorSizeStyleEdit);
        }
        ...
        {
            DeleteRecord(entry, "RBOCOLORGROUPTRANS", new[] { "Color", "Colorgroup" }, groupLineID, BusinessObjects.Permission.ColorSizeStyleEdit);
        }
        ...
        public virtual void SaveColor(IConnectionManager entry, ItemWithDescription color)
        {
