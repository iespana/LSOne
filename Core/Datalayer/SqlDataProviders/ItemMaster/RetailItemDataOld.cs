...
namespace LSOne.DataLayer.SqlDataProviders.ItemMaster
{
    ...
    /// </summary>
    public partial class RetailItemDataOld : SqlServerDataProviderBase, IRetailItemDataOld
    {
        ...
        protected virtual void PopulateItem(IDataReader dr, RetailItemOld item)
        {
            ...
            item.ItemType = (ItemTypeEnum)dr["ITEMTYPE"];

            // From RBOINVENTTABLE
            item.RetailItemType = (RetailItemOld.RetailItemTypeEnum)dr["RETAILITEMTYPE"];
            ...
            item.KeyInPrice = (RetailItemOld.KeyInPriceEnum)dr["KEYINGINPRICE"];
            item.KeyInQuantity = (RetailItemOld.KeyInQuantityEnum)dr["KEYINGINQTY"];
        ...
        protected virtual void PopulateModule(IDataReader dr, RetailItemOld.RetailItemModule module)
        {
            module.ItemID = (string)dr["ITEMID"];
            module.ModuleType = (RetailItemOld.ModuleTypeEnum)dr["MODULETYPE"];
        ...
        protected virtual void PopulateItemListItem(IDataReader dr, ItemListItemOld item)
        {
        ...
        protected virtual void PopulateAdvancedItemListItem(IDataReader dr, ItemListItemOld item)
        {
            item.ID = (string)dr["ITEMID"];
            item.Text = (string)dr["ITEMNAME"];
            item.NameAlias = (string)dr["NAMEALIAS"];
        ...
        protected virtual void PopulateAdvancedItemListItemWithRowCount(IDataReader dr, ItemListItemOld item)
        {
        ...
        public virtual RecordIdentifier GetItemUnitID(IConnectionManager entry, RecordIdentifier itemID, RetailItemOld.ModuleTypeEnum module)
        {
        ...
        /// sales)</param>
        public virtual void UpdateUnitID(IConnectionManager entry, RecordIdentifier itemID, RecordIdentifier unitID, RetailItemOld.ModuleTypeEnum module)
        {
        ...
        /// <summary>
        /// Searched for retail items that contain a given search text, and returns the results as a List of <see cref="ItemListItemOld"/>
        /// </summary>
        /// <param name="entry">Entry into the database</param>
        ...
        public virtual List<ItemListItemOld> Search(IConnectionManager entry, string searchString, int rowFrom, int rowTo, bool beginsWith, string sort)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                return Execute<ItemListItemOld>(entry, cmd, CommandType.Text, PopulateItemListItem);
            }
        }
        ...
        public List<ItemListItemOld> AdvancedSearchOptimized(IConnectionManager entry,
                                                                 ...
                                                                 RecordIdentifier specialGroup = null)
        {
            ...
            using (var cmdCount = entry.Connection.CreateCommand())
            {
                ...
                if (entry.Connection.DatabaseVersion != ServerVersion.SQLServer2012)
                {
                    ...
                    return Execute<ItemListItemOld>(entry, cmd, CommandType.Text, PopulateAdvancedItemListItem);
                }
                cmd.CommandText = cmd.CommandText.Replace("<RowCount>",
                    ",COUNT(1) OVER ( ORDER BY <sort> RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING ) Row_Count")
                    .Replace("<sort>", sort);
                   
                List<ItemListItemOld> result = Execute<ItemListItemOld>(entry, cmd, CommandType.Text,
        ...
        public List<ItemListItemOld> AdvancedSearch(IConnectionManager entry,
                                                        int rowFrom, int rowTo, string sort,
                                                        out int totalRecordsMatching,
                                                        string idOrDescription = null,
                                                        bool idOrDescriptionBeginsWith = true,
        ...
        /// <returns></returns>
        public virtual List<ItemListItemOld> AdvancedSearch(IConnectionManager entry, string searchString, int rowFrom, int rowTo, bool beginsWith, string sort, Dictionary<RetailItemSearchEnum, string> advancedSearchParameters)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                string modifiedSearchString = PreProcessSearchText(searchString, true, beginsWith);
        ...
        /// <summary>
        /// Searches for the given search text, and returns the results as a list of <see cref="ItemListItemOld" />
        /// </summary>
        /// <param name="entry">The entry into the database</param>
        ...
        public virtual RetailItemOld Get(IConnectionManager entry, RecordIdentifier itemID, CacheType cacheType = CacheType.CacheTypeNone)
        {
            RetailItemOld result;
            if (cacheType != CacheType.CacheTypeNone)
            {
                result = (RetailItemOld)entry.Cache.GetEntityFromCache(typeof (RetailItemOld), itemID);
                if (result != null)
                {
                    return result;
                }
            ...
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                var records = Execute<RetailItemOld>(entry, cmd, CommandType.Text, PopulateItem);
            ...
            if (result != null)
            {
                using (var cmd = entry.Connection.CreateCommand())
                {
                    ...
                    var moduleRecords = Execute<RetailItemOld.RetailItemModule>(entry, cmd, CommandType.Text, PopulateModule);

                    RetailItemOld.RetailItemModule inventModule = null;
                    bool purchaseUnitEmpty = false;
                    foreach (var module in moduleRecords)
                    {
                        if (module.ModuleType == RetailItemOld.ModuleTypeEnum.Purchase)
                            purchaseUnitEmpty = (module.Unit.IsEmpty || string.IsNullOrEmpty((string) module.Unit));
                        else if (module.ModuleType == RetailItemOld.ModuleTypeEnum.Inventory)
                            inventModule = module;
                    ...
                    {
                        var purchaseModule = inventModule.Clone() as RetailItemOld.RetailItemModule;
                        purchaseModule.ModuleType = RetailItemOld.ModuleTypeEnum.Purchase;
        ...
        /// <returns>The retail item, or null if not found</returns>
        public virtual List<RetailItemOld> GetItemsByItemPattern(IConnectionManager entry, string itemID)
        {
            List<RetailItemOld> records;
            ...
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                records = Execute<RetailItemOld>(entry, cmd, CommandType.Text, PopulateItem);
            }
        ...
        /// <param name="entry">Entry into the database</param>
        /// <returns>The retail item, or null if not found</returns>
        public virtual List<RetailItemOld> GetAllItems(IConnectionManager entry)
        {
            List<RetailItemOld> records;
        ...
        public virtual void Save(IConnectionManager entry, RetailItemOld item)
        {
            ...
            if (item.Dirty)
            {
                ...
                {
                    isNew = true;
                    item.ID = DataProviderFactory.Instance.GenerateNumber<IRetailItemDataOld, RetailItemOld>(entry); 
                }
            ...
            for (int i = 0; i < 3; i++)
            {
                var module = item[(RetailItemOld.ModuleTypeEnum)i];
        ...
        public virtual bool ItemHasDimentionGroup(IConnectionManager entry, ItemListItemOld itemID)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                cmd.CommandType = CommandType.Text;
        ...
        public virtual RetailItemOld.RetailItemModule GetPriceModule(IConnectionManager entry, RecordIdentifier itemID)
        {
            using(var cmd = entry.Connection.CreateCommand())
            {
                ...
                var list = Execute<RetailItemOld.RetailItemModule>(entry, cmd, CommandType.Text, PopulateModule);
        ...
        protected virtual void SaveCustomA(IConnectionManager entry, SqlServerStatement statement, RetailItemOld retailItem)
        {
        }

        protected virtual void SaveCustomB(IConnectionManager entry, SqlServerStatement statement, RetailItemOld retailItem)
        {
        }

        protected virtual void SaveCustomModule(IConnectionManager entry, SqlServerStatement statement, RetailItemOld.RetailItemModule module)
        {
        }
        #endregion
