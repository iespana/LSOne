	...
	public partial class SiteServiceService
	{
			...
			bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.GetJournalTransactionList(CreateLogonInfo(entry), journalTransactionId, sortBy,
				sortedBackwards, noExcludedInventoryItems), closeConnection);
		...
		{
			DoRemoteWork(entry, siteServiceProfile, () => server.ReserveStockTransaction(CreateLogonInfo(entry), reservation, storeID, typeOfTransactionLine), closeConnection);
		}
											 ...
											 string unitID, decimal adjustmentInventoryUnit, RecordIdentifier reasonCode, string reasonText, string reference, bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => server.ReserveStockItem(CreateLogonInfo(entry), itemID, storeID, adjustment, inventoryType, costPrice, netSalesPricePerItem, salesPricePerItem, unitID, adjustmentInventoryUnit, reasonCode, reasonText, reference), closeConnection);
		}
		...
		public virtual InventoryAdjustment GetInventoryAdjustment(IConnectionManager entry, SiteServiceProfile siteServiceProfile, RecordIdentifier masterID, bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.GetInventoryAdjustment(CreateLogonInfo(entry), masterID), closeConnection);
		...
		{
			bool result = false;
			DoRemoteWork(entry, siteServiceProfile, () => result = server.DeleteInventoryJournal(CreateLogonInfo(entry), journalId), closeConnection);
			...
			InventoryJournalTypeEnum journalType, bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.GetSumOfReservedItemByStore(itemID, storeID, inventoryUnitID, journalType, CreateLogonInfo(entry)), closeConnection);
			...
			bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.AdvancedSearch( CreateLogonInfo(entry), journalType, searchCriteria, sortBy, sortBackwards, rowFrom, rowTo, out count), closeConnection);
		...
		public virtual List<InventoryJournalTransaction> AdvancedSearch(IConnectionManager entry, SiteServiceProfile siteServiceProfile, InventoryJournalLineSearch searchCriteria, out int totalRecords, bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.AdvancedSearch(CreateLogonInfo(entry), searchCriteria, out count), closeConnection);
													...
													bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.UpdateStatus(CreateLogonInfo(entry), journalId, lineNum, masterId, newStatus), closeConnection);
																					...
																					bool closeConnection)
		{
			...
			DoRemoteWork(entry, siteServiceProfile, () => result = server.GetMovedToInventoryLines(CreateLogonInfo(entry), journalId, lineMasterId, sortBy, sortBackwards), closeConnection);
