...
namespace LSOne.Services
{
    ...
    internal class RunJobOperation
    {
        ...
        public RunJobOperation(Control control)
        {
        ...
        public void RunJobAsyncUnsecure(Guid jobid, string jobText, bool forceNormal = false, JscSchedulerLog jobLogID = null, JscLocation locationID = null)
        {
            lock (counterMonitor)
            {
                pendingJobCount++;
        ...
        private void RunJobThreadMain(object obj)
        {
            ...
            if (errorMsg != null)
            {
                DLLEntry.DataModel.ErrorLogger.LogMessage(LogMessageType.Error, errorMsg);
            }
            else
            {
                string msg;
        ...
        private void LogToDatabase(Guid jobId, string message)
        {
            ...
            DataProviderFactory.Instance.Get<IJobData, JscJob>().Save(DLLEntry.DataModel, logEntry);
        ...
        {
            var serverSettings = DLLEntry.SchedulerSettings.ServerSettings;
