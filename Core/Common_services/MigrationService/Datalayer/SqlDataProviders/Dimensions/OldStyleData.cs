...
namespace LSOne.Services.Datalayer.SqlDataProviders.Dimensions
{
    ...
    /// </summary>
    public class OldStyleData : SqlServerDataProviderBase, IOldStyleData
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OldStyleData"/> class.
        /// </summary>
        ...
        private static void PopulateStyle(IDataReader dr, OldItemWithDescription style)
        {
        ...
        public virtual OldItemWithDescription GetStyle(IConnectionManager entry, RecordIdentifier styleID)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                var items = Execute<OldItemWithDescription>(entry, cmd, CommandType.Text, PopulateStyle);
                return (items.Count > 0) ? items[0] : null;
            }
        }


        private static void PopulateStyleGroupLine(IDataReader dr, OldStyleGroupLineItem item)
        {
        ...
        public virtual List<OldStyleGroupLineItem> GetGroupLines(IConnectionManager entry, RecordIdentifier groupID, string sort)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                return Execute<OldStyleGroupLineItem>(entry, cmd, CommandType.Text, PopulateStyleGroupLine);
            }
        }
        ...
        public virtual OldStyleGroupLineItem GetGroupLine(IConnectionManager entry, RecordIdentifier groupID, RecordIdentifier styleID)
        {
            using (var cmd = entry.Connection.CreateCommand())
            {
                //ValidateSecurity(entry, BusinessObjects.Permission.CustomerView);
                ...
                var items = Execute<OldStyleGroupLineItem>(entry, cmd, CommandType.Text, PopulateStyleGroupLine);
        ...
        {
            DeleteRecord(entry, "RBOSTYLES", "STYLE", styleID, DataLayer.BusinessObjects.Permission.ColorSizeStyleEdit);
        }
        ...
        {
            DeleteRecord(entry, "RBOSTYLEGROUPTABLE", "STYLEGROUP", styleGroupID, DataLayer.BusinessObjects.Permission.ColorSizeStyleEdit);
            DeleteRecord(entry, "RBOSTYLEGROUPTRANS", "STYLEGROUP", styleGroupID, DataLayer.BusinessObjects.Permission.ColorSizeStyleEdit);
        }
        ...
        {
            DeleteRecord(entry, "RBOSTYLEGROUPTRANS", new[] { "STYLE", "Stylegroup" }, groupLineID, DataLayer.BusinessObjects.Permission.ColorSizeStyleEdit);
        }
        ...
        public virtual void SaveStyle(IConnectionManager entry, OldItemWithDescription style)
        {
