...
namespace LSOne.Services.Datalayer.SqlDataProviders
{
    ...
    /// </summary>
    public class OldSpecialGroupData : SqlServerDataProviderBase, IOldSpecialGroupData
    {
        private static string ResolveItemSort(OldSpecialGroupItemSorting sort, bool backwards)
        {
            switch (sort)
            {
                case OldSpecialGroupItemSorting.ItemID:
                    return "a.ITEMID" + (backwards ? " DESC" : " ASC");

                case OldSpecialGroupItemSorting.ItemName:
        ...
        private static string ResolveSort(OldSpecialGroupSorting sort, bool backwards)
        {
            switch (sort)
            {
                case OldSpecialGroupSorting.GroupID:
                    return "GROUPID"  + (backwards ? " DESC" : " ASC");

                case OldSpecialGroupSorting.GroupName:
        ...
        private static void PopulateItemInGroup(IDataReader dr, OldItemInGroup item)
        {
        ...
        public virtual List<DataEntity> GetList(IConnectionManager entry, OldSpecialGroupSorting sortBy, bool sortBackwords)
        {
            if (sortBy != OldSpecialGroupSorting.GroupID && sortBy != OldSpecialGroupSorting.GroupName)
            {
        ...
        public List<OldItemInGroup> ItemsNotInSpecialGroup(
            ...
            RecordIdentifier excludedGroupId)
        {
            ...
            using (var cmd = entry.Connection.CreateCommand())
            {
                ...
                return Execute<OldItemInGroup>(entry, cmd, CommandType.Text, PopulateItemInGroup);
            }
        }
        ...
        {
            DeleteRecord(entry, "RBOSPECIALGROUP", "GROUPID", specialGroupId, DataLayer.BusinessObjects.Permission.ManageSpecialGroups);
        }
        ...
        public virtual void RemoveItemFromSpecialGroup(IConnectionManager entry, RecordIdentifier itemId, RecordIdentifier groupId)
        {
            ...
            DeleteRecord(entry, "RBOSPECIALGROUPITEMS", new[] { "GROUPID", "ITEMID" }, recID, DataLayer.BusinessObjects.Permission.ManageSpecialGroups); 
        }
        ...
        public virtual void Save(IConnectionManager entry, DataEntity specialGroup)
        {
            ...
            ValidateSecurity(entry, DataLayer.BusinessObjects.Permission.ManageSpecialGroups);

            bool isNew = false;
            if (specialGroup.ID == RecordIdentifier.Empty)
            {
                isNew = true;
                specialGroup.ID = DataProviderFactory.Instance.GenerateNumber<IOldSpecialGroupData, DataEntity>(entry);
            }
        ...
        public virtual List<OldSpecialGroupItem> GetItemsGroupInformation(IConnectionManager entry, RecordIdentifier itemId)
        {
            var groups = GetList(entry);
            var groupResults = new List<OldSpecialGroupItem>();
            ...
            foreach (var group in groups)
            {
                ...
                groupResults.Add(new OldSpecialGroupItem { GroupId = group.ID, GroupName = group.Text, ItemIsInGroup = itemIsInGroup });
            }
