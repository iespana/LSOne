...
namespace LSOne.ViewPlugins.Scheduler.DataProviders
{
    public class LocationData : SqlServerDataProvider, ILocationData
    {
        ...
        /// <param name="entry"></param>
        private void HandleTerminal(Dictionary<string, JscLocation> existingLocations, JscLocation groupLocation, DataEntity terminal, IConnectionManager entry)
        {
            string terminalID = terminal.ID.ToString().Trim();
            string terminalIntegrationKey = MakeIntegrationKey(terminalID, LocationKind.Terminal);
            ...
            if (!existingLocations.TryGetValue(terminalIntegrationKey, out terminalLocation))
            {
                ...
                terminalLocation = CreateLocation(entry, entry.Connection.DataAreaId, terminalID, terminal.Text,
