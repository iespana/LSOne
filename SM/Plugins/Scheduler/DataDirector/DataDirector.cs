...
namespace LSOne.ViewPlugins.Scheduler.DataDirector
{
    internal sealed class DataDirector : IDisposable
    {
        ...
        private LSRetail.DD.Control.TransAutomClient privateDataClient;
        ...
        public void StartReadDesign(Guid locationId, bool readTablesAndFields, bool updateExistingDesign, string newDescription, RecordIdentifier designID =null)
        {
            ReadDesignDelegate worker = ReadDesign;
            ReadDesignDelegateDirect directWorker = ReadDesign;
            AsyncCallback completedCallback = ReadDesignCompletedCallback;
            AsyncCallback directCompletedCallback = ReadDesignDirectCompletedCallback;
            ...
            lock (monitor)
            {
                ...
                if (designID == null)
                {
                    worker.BeginInvoke(locationId, readTablesAndFields, updateExistingDesign, newDescription,
                        completedCallback, async);
                }
                else
                {
                    directWorker.BeginInvoke(designID, readTablesAndFields, updateExistingDesign, newDescription,
                    directCompletedCallback, async);
                }
        ...
        {            
            JscLocation location = DDProviders.LocationData.GetLocation(PluginEntry.DataModel, locationId);
            if (location == null)
            ...
            DDProviders.LocationData.Save(PluginEntry.DataModel, location);
            return reply;
        }
        ...
        private void ReadFieldDesign(Guid locationId, Guid tableDesignId)
        {
            ...
            JscTableDesign tableDesign = DDProviders.DesignData.GetTableDesign(PluginEntry.DataModel, tableDesignId);
            if (tableDesign == null)
            {
        ...
        private void PrepareDatabaseDesign(JscLocation location, bool updateExistingDesign, string newDescription, out IDictionary<string, TableDesignInfo> tableDesigns, out Guid databaseDesignId)
        {
            ...
            if (updateExistingDesign && location.DatabaseDesign != null)
            {
                ...
                foreach (var tableDesign in DDProviders.DesignData.GetTableDesigns(PluginEntry.DataModel, location.DatabaseDesign, false))
                {
            ...
            else
            {
                ...
                DDProviders.DesignData.Save(PluginEntry.DataModel, databaseDesign);
        ...
        private void PrepareDatabaseDesign(RecordIdentifier dDesign, bool updateExistingDesign, string newDescription, out IDictionary<string, TableDesignInfo> tableDesigns, out Guid databaseDesignId)
        {
            ...
            if (updateExistingDesign )
            {
                ...
                foreach (var tableDesign in DDProviders.DesignData.GetTableDesigns(PluginEntry.DataModel, dDesign, false))
                {
                    tableDesigns.Add(tableDesign.TableName, new TableDesignInfo { TableDesign = tableDesign });
                }
            }
        ...
        private void FetchAllTables(Guid databaseDesignId, IDictionary<string, TableDesignInfo> tableDesigns, float initProgress)
        {
            ...
            while (DataClient.NextValueList())
            {
                ...
                if (!tableDesigns.TryGetValue(tableName, out info))
                {
                    ...
                    DDProviders.DesignData.Save( PluginEntry.DataModel,tableDesign);
        ...
        private void ReadFieldsOfTable(Guid tableDesignId, string tableName)
        {
            ...
            foreach (var fieldDesign in DDProviders.DesignData.GetFieldDesigns(PluginEntry.DataModel, tableDesignId))
            {
            ...
            while (DataClient.NextValueList())
            {
                ...
                if (fieldDesigns.TryGetValue(fieldName, out fieldDesignInfo))
                {
                    ...
                    if(dataIsModified)
                    {
                        DDProviders.DesignData.Save(PluginEntry.DataModel, fieldDesignInfo.FieldDesign);
                    }
                }
                else
                {
            ...
            foreach (string field in fieldDesigns.Keys.Where(p => !currentFields.Contains(p)))
            {
                DDProviders.DesignData.DeleteFieldDesign(PluginEntry.DataModel, fieldDesigns[field].FieldDesign.ID);
            }
