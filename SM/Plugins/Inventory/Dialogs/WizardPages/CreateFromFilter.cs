...
{
    public partial class CreateFromFilter : UserControl, IWizardPage
    {
        private WizardBase parent;
        private InventoryTypeAction inventoryTypeAction;
        private InventoryTemplateFilterContainer filter;

        public CreateFromFilter(WizardBase parent, InventoryTypeAction inventoryTypeAction)
        {
            InitializeComponent();
            ...
            foreach (InventoryTemplateFilterListItem item in items)
            {
                ...
                if (isAssemblyItem)
                {
                    var button = new IconButton(
                        PluginEntry.Framework.GetImage(ImageEnum.EmbeddedInformation), 
                        Resources.AssemblyItemWillBeReplacedWithComponents, true);
        ...
        public IWizardPage RequestNextPage()
        {
            ...
            {
                case InventoryEnum.PurchaseOrder:
                    return new NewEmptyPO(parent, filter, inventoryTypeAction);
                case InventoryEnum.StockCounting:
                    return new NewEmptySCJ(parent, filter, inventoryTypeAction);
                default:
                    return null;
            }
        }
        ...
        private void searchBar_SetupConditions(object sender, EventArgs e)
        {
            ...
            if(inventoryTypeAction.InventoryType == InventoryEnum.StockCounting)
            {
                searchBar.AddCondition(new ConditionType(Resources.InventoryOnHand, "InventoryOnHand", ConditionType.ConditionTypeEnum.Numeric));
            }
        }
