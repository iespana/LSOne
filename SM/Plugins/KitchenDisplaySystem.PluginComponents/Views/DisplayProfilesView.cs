...
{
    public partial class DisplayProfilesView : ViewBase
    {
        RecordIdentifier selectedProfileId;

        public DisplayProfilesView(RecordIdentifier selectedProfileId)
            :this()
        {
            this.selectedProfileId = selectedProfileId;
        }

        public DisplayProfilesView()
        {
            InitializeComponent();
            ...
            HeaderText = Properties.Resources.DisplayProfiles;
        ...
        protected override string LogicalContextName
        {
            get
            {
                return Properties.Resources.DisplayProfiles;
        ...
        {
            contexts.Add(new AuditDescriptor("KITCHENDISPLAYPROFILESLog", RecordIdentifier.Empty, Properties.Resources.DisplayProfiles, false));
        }
        ...
        public override void OnDataChanged(DataEntityChangeType changeAction, string objectName, RecordIdentifier changeIdentifier, object param)
        {
            switch (objectName)
            {
                case "KitchenDisplayProfile":
                    if (changeAction == DataEntityChangeType.Delete)
                    {
                        //TO DO - Close the pages and refresh the list
                    }
                    LoadData(false);
                    break;
            }
        ...
        private void LoadProfiles()
        {
            ...
            List<KitchenDisplayProfile> profiles = Providers.KitchenDisplayProfileData.GetList(PluginEntry.DataModel);

            foreach (var profile in profiles)
            {
                listItem = new ListViewItem(profile.Text);
                listItem.SubItems.Add(KitchenDisplayStation.GetDisplayModeText(profile.DisplayMode));
        ...
        private void btnsEditAddRemove_EditButtonClicked(object sender, EventArgs e)
        {
            ...
            PluginOperationsHelper.ShowDisplayProfileView(selectedId);
        }
        ...
        private void btnsEditAddRemove_AddButtonClicked(object sender, EventArgs e)
        {
            ...
            {
                PluginOperationsHelper.ShowDisplayProfileView(dlg.Id);
            }          
        }
        ...
        private void btnsEditAddRemove_RemoveButtonClicked(object sender, EventArgs e)
        {
            ...
            {
                PluginOperationsHelper.DeleteDisplayProfile((RecordIdentifier)lvProfiles.SelectedItems[0].Tag);
            }
            else
            {
                PluginOperationsHelper.DeleteDisplayProfiles(lvProfiles.GetSelectedIDs());
            }
        }
