...
{
    public partial class PayCardTypeDropdownDialog : TouchBaseForm
    {
        ...
        public PayCardTypeDropdownDialog()
        {
            InitializeComponent();
            cardTypes = new List<StorePaymentTypeCardType>();
            ntbAmount.GhostText = Properties.Resources.Amount;
            ShowCardNumberField = false;
            paymentTypeID = RecordIdentifier.Empty;
        }

        public PayCardTypeDropdownDialog(decimal balanceAmount)
            : this()
        {
        ...
        private void Pay()
        {
            ...
            var roundedAmount = rounding.RoundAmount(DLLEntry.DataModel, (decimal)ntbAmount.Value, DLLEntry.DataModel.CurrentStoreID, tenderInformation.ID.SecondaryID, CacheType.CacheTypeTransactionLifeTime);
            if (((roundedAmount > 0 && balanceAmount >= 0) || (roundedAmount < 0 && balanceAmount < 0)) && ntbAmount.Text != "")
            {
                CaptureCardInfo();
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!ShowCardNumberField)
            {
                ...
                Size = new Size(Size.Width, Size.Height - tbCardNumber.Height - 37);
            }
        ...
        private void OnMsrMessageEvent(string track2)
        {
            try
            {
                ...
                if (cardInformation.TenderTypeId != "")
                {
                    ...
                    touchScrollButtonPanel1.SetButtonsCurrency(balanceAmount, tenderInformation, DLLEntry.Settings.Store.Currency);
